...
let code_key_map: Map<number, KeyInfo> = new Map()
let char_code_map: Map<string, number> = new Map()
        ...
        slots.forEach((slot, slot_index) => {
            ...
            if (looking_code !== undefined) {
                ...
                if (slot.cover.length > 0) {
                    let grammars: [GrammarName, number][] = slot.cover.map(name => {
                    ...
                    if (skip_info !== undefined) {
                        ...
                        const keygroup = key_grouping.get(numbers)

                        console.log("key_grouping", key_grouping)
                        console.log("key_group", keygroup)
                        if (keygroup !== undefined) {
                            const this_fj = looking_code > 9
                            if (keygroup.fjfree && last_ronrisetu_fj === this_fj) {
                            ...
                            pattern_items.forEach(([name, _], i) => {
                                const rev_i = pattern_items.length - 1 - i
                                if (name === "skip" && has_taku) {
                                    skips_group_id = ids[i]
